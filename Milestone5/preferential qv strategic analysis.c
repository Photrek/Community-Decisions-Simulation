	...
	{		
		...
		if(!isnan(totalOpinion) && totalOpinion > 0)
		{
			...
			while(proposalIndex < Nproposals && proposalsToVote[i][proposalIndex] != -1)
			{
				...
				if(voting_rating[i][proposal] > 0)
				{
					proposalVotes[proposal] += sqrt(voting_rating[i][proposal]*sqrt(money[i])/totalOpinion);
	...
	{		
		...
		if(!isnan(totalOpinion) && totalOpinion > 0)
		{
			...
			while(proposalIndex < Nproposals && proposalsToVote[i][proposalIndex] != -1)
			{
				...
				proposalVotes[proposal] += sign(voting_rating[i][proposal])*sqrt(fabsl(voting_rating[i][proposal])*sqrt(money[i])/totalOpinion);
